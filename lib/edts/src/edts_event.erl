...
-callback format_info(edts_events:class(), edts_events:type(), term()) ->
  [{atom(), term()}].
...
-record(
  state,
  {
    ...
    formatters = [] :: [{class(), module()}] | [{{class(), type()}, module()}]
  }
).
...
listen() -> gen_server:call({global, ?SERVER}, listen, infinity).

dispatch(Class, Type) -> dispatch(Class, Type, []).
...
state() -> gen_server:call({global, ?SERVER}, state).
...
-spec init(term()) ->
  {ok, state()} | {ok, state(), timeout()} | ignore | {stop, term()}.

%%------------------------------------------------------------------------------

init([Formatters]) -> {ok, #state{formatters = Formatters}}.
...
handle_call(Message, _From, State) -> {reply, {unknown_call, Message}, State}.
...
handle_cast({dispatch, Event0}, #state{listeners = Listeners0} = State0) ->
...
safe_fmt_event_info(Fmt, Class, Type, Info) ->
  try Fmt:format_info(Class, Type, Info) catch
     ?EXCEPTION(C, E, S) ->
      edts_log:error(
        "edts_event: Formatter ~p failed with ~p:~p.~n"
        "Class: ~p~n"
        "Type: ~p~n"
        "Info: ~p~n"
        "Stacktrace: ~p~n",
