...
expand_code_paths(ProjectRoot, LibDirs) ->
  RootPaths =
    [filename:join(ProjectRoot, "ebin"), filename:join(ProjectRoot, "test")],
...
lib_and_app_dirs() ->
  ...
  lists:partition(
    fun (Path) -> lists:prefix(ErlLibDir, Path) end,
    code:get_path()
  ).
...
project_lib_file_p(ProjectDir, LibDirs, File) ->
  lists:any(
    fun
      (LibDir) -> directory_child_p(filename:join(ProjectDir, LibDir), File)
    end,
    LibDirs
  )
  orelse
  lists:any(
    fun (Dir) -> filename:join(ProjectDir, Dir) =:= filename:dirname(File) end,
    ["src", "test", "ebin", ""]
  ).
...
expand_code_path_test() ->
  ...
  ?assertEqual(
    [
      filename:join([Root, "lib", "foo", "ebin"]),
