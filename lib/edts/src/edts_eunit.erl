...
-export_type([info/0, result/0, ok/0, error/0, summary/0, test/0, reason/0]).

%%%_* API ======================================================================
%%------------------------------------------------------------------------------
%% @doc Run eunit tests on Module and return result as "issues".

-spec run_tests(module()) -> {ok, [edts_code:issue()]} | {error, term()}.

%%------------------------------------------------------------------------------

run_tests(Module) -> try try_run_tests(Module) catch _:E -> {error, E} end.
...
filter_module_tests(Module, Tests) ->
  Fun =
    fun
      ({_Type, TestModule, _Fun} = Test, Acc) when TestModule =:= Module ->
        [Test | Acc];
...
do_run_tests(Ref, Listener, Timeout) ->
  ...
  receive {start, Ref} -> Listener ! {start, Ref} end,
...
format_results(Source, Results) ->
  lists:map(
    fun (Result) -> format_successful(Source, Result) end,
    orddict:fetch(successful, Results)
  )
  ++
  lists:map(
    fun (Result) -> format_failed(Source, Result) end,
    orddict:fetch(failed, Results)
  )
  ++
  lists:map(
    fun (Result) -> format_cancelled(Source, Result) end,
...
format_error({error, {Reason, Info} = Err, _Stack}) ->
  case reason_to_props(Reason) of
    ...
    {error, not_found} -> io_lib:format("~p", [Err])
  end;

format_error({Err, Reason, _Stack}) -> io_lib:format("~p", [{Err, Reason}]);
format_error(Err) -> io_lib:format("~p", [Err]).
...
get_line(Result) ->
  case proplists:get_value(line, Result) of
    Line when is_integer(Line) andalso Line > 0 -> Line;
    _ -> try get_error_line(Result) catch
        _:_ ->
    ...
    [
      {
        ...
        {pattern, [unexpected_success, unexpected_exception]}
      },
...
format_results_test_() ->
  ErrorInfo = [{expected, foo}, {value, bar}],
  PropList =
    [
      {successful, [[{line, 1}]]},
      {
