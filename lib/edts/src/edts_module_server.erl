...
-export([get_modules/0, start/0, start_link/0, started_p/0, update/0]).
...
init(_Args) ->
  ...
  {ok, #state{modules = fetch_modules(), timerref = TRef}}.
...
fetch_modules() ->
  ...
  ordsets:to_list(
    ordsets:union(ordsets:from_list(Loaded), ordsets:from_list(AppMods))
  ).
