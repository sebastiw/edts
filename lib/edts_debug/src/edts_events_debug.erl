...
format_info(edts_debug, starting, _) -> [{type, starting}];
format_info(edts_debug, _Type, Event) -> format_info(Event).

%%%_* Internal functions ======================================================

format_info({Type, Mod}) when Type =:= interpret orelse Type =:= no_interpret ->
    [{type, Type}, {module, Mod}];
...
format_info({new_process, {Pid, {Mod, Fun, Args}, Status, Info}}) ->
    [
        ...
        {function, Fun},
        {
            args,
            [
                list_to_binary(lists:flatten(io_lib:format("~w", [A])))
                || A <- Args
            ]
        },
        {status, Status},
...
format_info({new_status, Pid, Status, Info}) ->
    ...
    [{type, new_status}, {pid, edts_util:pid2atom(Pid)}, {status, Status}]
    ++
    InfoProps;

format_info({Type, {{Mod, Line}, Options}})
when Type =:= new_break orelse Type =:= break_options ->
    [{type, Type}, {module, Mod}, {line, Line}, {options, Options}];

format_info({delete_break, {Mod, Line}}) ->
    [{type, delete_break}, {module, Mod}, {line, Line}];

format_info({no_break, Mod}) -> [{type, no_break}, {module, Mod}].
