...
-export([execute/3, dirs/0, names/0, specs/0, to_ret_str/1, to_ret_str/3]).

%% Callbacks

-export(
  [
    edts_server_services/1,
    event_formatters/1,
...
-spec execute(Plugin :: module(), Cmd :: atom(), ctx()) ->
  {ok, [{result, ok | error} | {return, [{atom(), binary()}]}]}
  | {error, {badrpc, term()}}
  |
  {
    error,
    {not_found, [{plugin, Plugin :: module()} | {command, Cmd :: atom()}]}
  }.
execute(Plugin, Cmd, Input0) ->
  ...
  case cmd_exists_p(Plugin, Cmd, orddict:size(Input)) of
    true -> do_execute(Node, Plugin, Cmd, Input);
...
names() ->
  [Name || Dir <- dirs(), edts /= (Name = list_to_atom(filename:basename(Dir)))].

specs() -> lists:map(fun do_spec/1, dirs()).

to_ret_str(Term) -> list_to_binary(lists:flatten(io_lib:format("~p", [Term]))).
...
edts_server_services(Plugin) -> Plugin:edts_server_services().

event_formatters(Plugin) -> Plugin:event_formatters().

project_node_modules(Plugin) -> Plugin:project_node_modules().

project_node_services(Plugin) -> Plugin:project_node_services().

spec(Plugin, Cmd, Arity) -> Plugin:spec(Cmd, Arity).

%%%_* Internal functions =======================================================

cmd_exists_p(Plugin, Cmd, Arity) ->
  code:which(Plugin) =/= non_existing
  andalso lists:member({Cmd, Arity}, Plugin:module_info(exports)).

do_execute(Node, Plugin, Cmd, Input) ->
  edts_log:debug(
    "Validating input for ~p command ~p:~n~p",
    [Plugin, Cmd, Input]
  ),
  ...
  case edts:call(Node, Plugin, Cmd, Ctx) of
    ...
    {ok, {error, E}} -> {ok, [{result, error}, {return, to_ret_str(E)}]};
    %% All is well
    {ok, {ok, Ret}} -> {ok, [{result, ok}, {return, convert_return(Ret)}]};
    %% Local calls
    {ok, ok} -> {ok, [{result, ok}]};
    {ok, Ret} -> {ok, [{result, ok}, {return, convert_return(Ret)}]}
  end.
...
convert_return(Ret) when is_list(Ret) ->
  IsProp = fun ({K, _V}) when is_atom(K) -> true; (_) -> false end,
