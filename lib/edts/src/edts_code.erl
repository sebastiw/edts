...
-spec compile_and_load(Module :: file:filename() | module()) ->
  {ok, {[], [issue()]}} | {error, {[issue()], [issue()]}} | {error, term()}.
...
compile_and_load(File0, Opts) ->
  {ok, Cwd} = file:get_cwd(),
  File =
    case lists:prefix(Cwd, File0) of
  ...
  CompileOpts =
    [{cwd, Cwd}, {outdir, OutDir}, binary, debug_info, return | Opts]
    ++
    OldOpts
    ++
    AdditionalIncludes,
...
form_to_mfa({op, _, '/', {atom, _, F}, {integer, _, A}}) -> {F, A};

form_to_mfa(
  {op, _, '/', {remote, _, {atom, _, M}, {atom, _, F}}, {integer, _, A}}
) ->
  {M, F, A};

form_to_mfa({'fun', _, {function, F, A}}) -> {F, A};

form_to_mfa({'fun', _, {function, {atom, _, M}, {atom, _, F}, {integer, _, A}}}) ->
  {M, F, A};
...
form_to_mfa({call, _, {remote, _, {atom, _, M}, {atom, _, F}}, Args}) ->
  {M, F, length(Args)}.
...
-spec get_module_info(M :: module(), Level :: basic | detailed) ->
  {ok, [{atom(), term()}]} | {error, {not_found, module()}}.
...
do_get_module_info(M, basic) ->
  try erlang:get_module_info(M) of
    ...
    error : badarg -> {error, {not_found, M}}
  end;
...
project_specific_data_file(Suffix) ->
  ...
  Filename =
    io_lib:format("~p_~s~s", [erlang:phash2(RootDir), ProjectName, Suffix]),
...
started_p() -> edts_module_server:started_p().
...
-spec ensure_module_loaded(Reload :: boolean(), Mod :: module()) ->
  boolean() | {error, term()}.
...
reload_if_modified(SrcFile, OutDir) ->
  ...
  case code:is_loaded(Module) of
    {file, _Loaded} -> do_reload_if_modified(Module, Beam);
    false -> false
  end.
...
try_load_mod(Mod, File) ->
  ...
  try load_mod(Mod, LoadFileName) catch
    C:E ->
...
module_modified_mtime_p(Mod, File) ->
  do_module_modified_mtime_p(
    lists:keyfind(time, 1, Mod:module_info(compile)),
    file:read_file_info(File)
  ).

do_module_modified_mtime_p(false, _) -> true;
do_module_modified_mtime_p(_, {error, _}) -> true;
...
get_compile_outdir(File) ->
  ...
  Opts = try proplists:get_value(options, Mod:module_info(compile), []) catch
      error : undef ->
        %% No beam-file
        [] end,
...
get_module_source(M) -> get_module_source(M, M:module_info()).
...
-spec get_file_and_line(
  ...
  {ok, {File :: string(), Line :: non_neg_integer()}} | {error, not_found}.
...
get_file_and_line(_M, _F, _A, _CurFile, []) -> {error, not_found}.
...
parse_abstract({attribute, _Line, import, {Module, Imports0}}, Acc) ->
  Imports =
    [[{module, Module}, {function, F}, {arity, A}] || {F, A} <- Imports0],
...
path_flatten([], _Back, Acc) -> filename:join(["." | lists:reverse(Acc)]);
path_flatten(["." | Rest], Back, Acc) -> path_flatten(Rest, Back, Acc);
...
path_flatten([_Dir | Rest], [_ | Back], Acc) -> path_flatten(Rest, Back, Acc);
path_flatten([Dir | Rest], Back, Acc) -> path_flatten(Rest, Back, [Dir | Acc]).
...
get_app_includes(FileLoc) ->
  ...
  case
  filename:basename(FileLoc) =:= "src"
  orelse filename:basename(FileLoc) =:= "test" of
...
get_additional_includes_test_() ->
  ...
  AbsProjectInc = filename:join(Root, "test/include"),
  {
    ...
    [
      ...
      ?_assertEqual(
        [{i, AbsProjectInc}],
        get_additional_includes(SrcDir, [{i, filename:join(AppDir, "foo")}])
      )
    ]
  }.
...
string_to_mfa_test_() ->
  [
    ?_assertEqual({ok, [{function, foo}, {arity, 0}]}, string_to_mfa("foo()")),
    ?_assertEqual(
      {ok, [{function, foo}, {arity, 1}]},
    ...
    ?_assertEqual({ok, [{function, foo}, {arity, 1}]}, string_to_mfa("foo/1.")),
    ?_assertEqual(
      {ok, [{module, foo}, {function, bar}, {arity, 1}]},
      string_to_mfa("foo:bar/1.")
    ),
    ?_assertEqual(
      {ok, [{function, 'MATCH'}, {arity, 0}]},
      string_to_mfa("MATCH.")
    ),
    ?_assertEqual(
      {ok, [{function, 'MATCH'}, {arity, 0}]},
    ...
    ?_assertEqual(
      {error, [{error, "Snippet", 1, "syntax error before: '.'"}]},
      string_to_mfa("foo(\"aa,bb\".")
    )
  ].
...
format_errors_test_() ->
  ...
  CleanupF = fun (_) -> meck:unload() end,
  {
    setup,
    SetupF,
    CleanupF,
    [
      ?_assertEqual(
        ...
        format_errors(
          warning,
          [
            {"file1", [{1337, source, error1}, {1338, source, error2}]},
            {"file2", [{1339, source, error3}, {1340, source, error4}]}
          ]
        )
      ),
      ?_assertEqual(
        [
          {error, "file1", 1337, "error1"},
...
get_file_and_line_test_() ->
  ...
  [
    ?_assertEqual({error, not_found}, get_file_and_line(Mod, f, 0, File, [])),
    ?_assertEqual({ok, {File, 12}}, get_file_and_line(Mod, foo, 0, File, Forms)),
    ?_assertEqual(
      {ok, {"bar.erl", 12}},
      get_file_and_line(
        Mod,
...
get_file_and_line_parametrised_mod_test_() ->
  ...
  [
    ?_assertEqual({ok, {File, 3}}, get_file_and_line(Mod, new, 1, File, Forms)),
    ?_assertEqual({ok, {File, 10}}, get_file_and_line(Mod, foo, 0, File, Forms)),
    ?_assertEqual({ok, {File, 12}}, get_file_and_line(Mod, new, 0, File, Forms)),
    ?_assertEqual({ok, {File, 14}}, get_file_and_line(Mod, new, 2, File, Forms))
  ].
...
get_file_and_line_non_parametrised_new_test_() ->
  ...
  [
    ...
    ?_assertEqual({ok, {File, 12}}, get_file_and_line(Mod, foo, 0, File, Forms)),
    ?_assertEqual({ok, {File, 14}}, get_file_and_line(Mod, new, 0, File, Forms)),
    ?_assertEqual({ok, {File, 16}}, get_file_and_line(Mod, new, 2, File, Forms))
  ].
...
get_module_source_test_() ->
  ...
  ErlangAbsNames =
    [
      ...
      filename:join(edts_util:shorten_path(code:lib_dir(edts, src)), BaseName),
      filename:join(
        edts_util:shorten_path(code:lib_dir(edts, '.eunit')),
        BaseName
      )
    ],
  [
    ?_assertEqual(
      {error, not_found},
      module_source_test_ret(get_module_source_from_info(erlang:module_info()))
    ),
    ?_assert(
      lists:member(
        module_source_test_ret(get_module_source_from_beam(?MODULE)),
        ErlangAbsNames
      )
    ),
    ?_assertEqual(
      {error, not_found},
      module_source_test_ret(get_module_source_from_info([]))
    ),
    ?_assertEqual(
      {error, not_found},
      module_source_test_ret(get_module_source_from_beam(erlang_foo))
    ),
    ?_assert(
      lists:member(
    ...
    ?_assertEqual(
      {error, not_found},
      module_source_test_ret(get_module_source(erlang_foo, []))
    )
  ].
...
parse_abstract_function_test_() ->
  ...
  Acc =
    orddict:from_list(
      [{module, Mod}, {cur_file, CurFile}, {exports, Exports}, {functions, []}]
    ),
...
parse_abstract_include_test_() ->
  ...
  Acc =
    orddict:from_list(
      [{source, Source0}, {compile_cwd, CompileCwd}, {includes, []}]
    ),
...
parse_abstract_import_test_() ->
  ...
  [
    ...
    ?_assertEqual(
      lists:sort([{module, foo}, {function, bar}, {arity, 1}]),
...
parse_abstract_record_test_() ->
  Fields =
    [
      {record_field, foo, {foo, foo, field_1}},
      {record_field, foo, {foo, foo, field_2}, init_call},
      {typed_record_field, {record_field, foo, {foo, foo, field_3}}, type},
      {
        typed_record_field,
        {record_field, foo, {foo, foo, field_4}, init_call},
        type
      }
    ],
  Line = 1337,
  CurFile = "/foo/test.erl",
  Acc = orddict:from_list([{cur_file, CurFile}, {records, []}]),
...
modules_test_() -> [?_assertMatch([_ | _], modules())].
      ...
      fun
        () ->
          ...
          meck:expect(
            filelib,
            is_dir,
            fun ("good/../ebin") -> true; (_) -> false end
          )
      end,
