...
start() ->
    ...
    try
        case read_file(File) of
            {ok, BinState} ->
                error_logger:info_msg(
                    "Found previous state to start from in ~p.",
                    [File]
                ),
        ...
        C:E ->
            error_logger:error_msg(
                "Starting xref from ~p failed with: ~p:~p~n~n"
...
do_start() -> do_start_with(fun () -> xref:start(?MODULE) end).
...
check_modules(Modules0, Checks) ->
    ...
    Files =
        [proplists:get_value(source, M:module_info(compile)) || M <- Modules],
            ...
            ({{{Mod, _, _}, {CM, CF, CA}}, Lines}) ->
                case lists:keyfind(Mod, 1, ModuleFiles) of
                    ...
                    {Mod, File} ->
                        Desc =
                            io_lib:format(
                                "Call to undefined function ~p:~p/~p",
                                [CM, CF, CA]
                            ),
                        [
            ...
            ({{M, F, A}, Line}) ->
                case ignored_p(M, F, A) of
                    ...
                    false ->
                        ...
                        Desc =
                            io_lib:format("Unused export ~p:~p/~p", [M, F, A]),
                        [{error, File, Line, lists:flatten(Desc)}]
                end
        end,
    lists:flatmap(FmtFun, Res).
...
ignored_p(M, F, A) ->
    ignored_test_fun_p(M, F, A)
    orelse lists:member({M, F, A}, get_xref_ignores(M)).
...
update_modules(Modules) ->
    Added =
        ordsets:from_list([M || {M, _Props} <- xref:info(?SERVER, modules)]),
    ToAdd0 =
        ordsets:to_list(ordsets:subtract(ordsets:from_list(Modules), Added)),
            ...
            lists:flatmap(
                fun
                    (Dir) ->
                        ...
                        [
                            {list_to_atom(filename:basename(B, ".beam")), B}
                            || B <- Beams
                        ]
                end,
                AppDirs
                ...
                case dict:find(Mod, ModsLoaded) of
                    ...
                    {ok, Info} ->
                        ...
                        case proplists:get_value(directory, Info) of
                            ...
                            _OldDir ->
                                % Mod has moved
                                xref:replace_module(?SERVER, Mod, Beam)
                        end
                end
        end,
...
try_add_module(Mod, Beam) ->
    try
        ...
        case Opts =/= undefined andalso lists:member(debug_info, Opts) of
            ...
            false ->
                error_logger:error_msg(
                    "xref can't add ~p: no debug-info",
                    [Mod]
                ),
                {error, {no_beam, Mod}}
        end
...
get_xref_ignores(Mod) ->
    F =
        fun
            ({ignore_xref, Ignores}, Acc) -> Ignores ++ Acc;
            (_, Acc) -> Acc
        end,
...
xref_file() -> edts_code:project_specific_data_file(".xref").
...
check_undefined_function_calls_test() ->
    ...
    ?assertEqual(
        [],
        check_modules([edts_test_module], [undefined_function_calls])
    ),
    ?assertMatch(
        [{error, _File, _Line, Str}]
        when is_list(Str),
             check_modules([edts_test_module2], [undefined_function_calls])
    ),
    teardown_eunit().
...
check_unused_exports_test() ->
    ...
    ?assertMatch(
        [{error, _File2, _Line1, Str1}, {error, _File2, _Line2, Str2}]
        when is_list(Str1) andalso is_list(Str2),
