...
-export([check_exists_and_do_rpc/3, exists_p/2, validate/2]).
    ...
    fun
      (Key, Ctx) ->
        ...
        case (term_to_validate(Key)) (Ctx) of
          {ok, Value} -> orddict:store(Name, Value, Ctx);

          {error, Rsn} ->
            edts_log:error(
              "API input validation failed. Key ~p, Rsn: ~p",
              [Key, Rsn]
            ),
            throw({error, Key})
        end
    end,
  try {ok, lists:foldl(F, Ctx0, Keys)} catch
    throw : {error, Key0} = E ->
      Key =
        case Key0 of
...
term_to_validate(code) -> fun (Ctx) -> string_validate(Ctx, code) end;
term_to_validate(string) -> fun (Ctx) -> string_validate(Ctx, string) end;
...
term_to_validate({enum, Props}) -> fun (Ctx) -> enum_validate(Ctx, Props) end;
...
term_to_validate({file, Key}) -> fun (Ctx) -> file_validate(Ctx, ?a2l(Key)) end;
...
term_to_validate(line) -> fun (Ctx) -> non_neg_integer_validate(Ctx, line) end;
...
-spec arity_validate(orddict:orddict()) ->
  {ok, non_neg_integer()} | {error, {badarg, string()}}.
...
-spec integer_validate(orddict:orddict(), atom()) ->
  {ok, integer()} | {error, {badarg, string()}}.
...
-spec enum_list_validate(orddict:orddict(), [{atom(), term()}]) ->
  {ok, [atom()]} | {error, {illegal, [atom()]}} | {error, {not_found, atom()}}.
...
-spec enum_validate(orddict:orddict(), [{atom(), term()}]) ->
  {ok, atom()} | {error, {illegal, string()}} | {error, {not_found, atom()}}.
...
enum_validate(Ctx, Props) ->
  ...
  case orddict:find(Name, Ctx) of
    error when Required -> {error, {not_found, Name}};
    error -> {ok, edts_util:assoc(default, Props, undefined)};
...
-spec file_validate(orddict:orddict()) ->
  {ok, filename:filename()} | {error, {no_exists, string()}}.

%%------------------------------------------------------------------------------

file_validate(Ctx) -> file_validate(Ctx, file).
...
function_validate(Ctx) -> {ok, list_to_atom(orddict:fetch(function, Ctx))}.
...
module_validate(Ctx) -> {ok, list_to_atom(orddict:fetch(module, Ctx))}.
...
nodename_exists_p(Ctx) -> edts:node_reachable(orddict:fetch(nodename, Ctx)).
...
-spec project_root_validate(orddict:orddict()) ->
  {ok, file:filename()} | {error, {not_dir, string()}}.
...
dirs_validate_validate_test() ->
  ...
  ?assertEqual({ok, [LibDir, LibDir]}, dirs_validate(Dirs, project_lib_dirs)),
  ?assertEqual({ok, []}, dirs_validate([], project_lib_dirs)).


module_validate_test() ->
  ?assertEqual({ok, foo}, module_validate(orddict:from_list([{module, "foo"}]))).
...
nodename_validate_test() ->
  ...
  ?assertEqual({ok, list_to_atom("foo@" ++ Hostname)}, nodename_validate(Ctx)).
...
project_root_validate_test() ->
  ...
  ?assertEqual({error, {not_dir, Path}}, project_root_validate(Ctx2)).
...
enum_validate_test() ->
  ...
  ?assertEqual(
    ...
    enum_validate(
      Ctx0,
      [{name, a}, {allowed, [a, b]}, {default, a}, {required, false}]
    )
  ),
  ...
  ?assertEqual(
    ...
    enum_validate(
      Ctx1,
      [{name, a}, {allowed, [a, b]}, {default, b}, {required, false}]
    )
  ),
  ...
  ?assertEqual(
    ...
    enum_validate(
      Ctx1,
      [{name, a}, {allowed, [a, b]}, {default, b}, {required, true}]
    )
  ),
  %% Illegal value
...
enum_list_validate_test() ->
  ...
  ?assertEqual(
    ...
    enum_list_validate(
      Ctx1,
      [{name, undefined}, {allowed, [valid_1, valid_2]}, {required, false}]
    )
  ),
  %% Required value not found
  ?assertEqual(
    {error, {not_found, valid}},
    enum_list_validate(
      Ctx1,
      [{name, valid}, {allowed, [valid_1, valid_2]}, {required, true}]
    )
  ),
  %% One illegal value
