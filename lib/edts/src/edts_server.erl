...
-export(
  [init_node/7, node_registered_p/1, nodes/0, start_link/0, start_service/2]
).
...
nodes() -> gen_server:call(?SERVER, nodes, infinity).
...
-spec init(list()) ->
  {ok, state()} | {ok, state(), timeout()} | ignore | {stop, atom()}.
...
-spec code_change(OldVsn :: string(), state(), Extra :: term()) ->
  {ok, state()}.
...
do_init_node(
  ...
  try
    ...
    ok =
      lists:foreach(
        fun (Spec) -> edts_dist:load_app(Node, Spec) end,
        edts_plugins:specs()
      ),
    PluginRemoteLoad =
    ...
    start_services(Node, [edts_code] ++ PluginRemoteServices)
     ...
     ?EXCEPTION(C, E, S) ->
      edts_log:error(
        "~p initialization crashed with ~p:~p~nStacktrace:~n~p",
...
start_service(Node, Service) ->
  ...
  case edts_dist:start_service(Node, Service) of
    ...
    {error, _} = Err ->
      edts_log:error(
        "Starting service ~p on ~p failed with ~p",
        [Service, Node, Err]
      ),
      Err
  end.
...
node_find(Name, State) -> lists:keyfind(Name, #node.name, State#state.nodes).
...
init_node_test() ->
  ...
  meck:new(edts_dist),
  meck:expect(edts_dist, add_paths, fun (foo, _) -> ok; (bar, _) -> ok end),
  meck:expect(edts_dist, load_app, fun (_, _) -> ok end),
  meck:expect(edts_dist, init_node, fun (_, _) -> ok end),
  meck:expect(
    edts_dist,
    remote_load_modules,
    fun (foo, _) -> ok; (bar, _) -> ok end
  ),
  meck:expect(edts_dist, set_cookie, fun (foo, cookie) -> ok end),
  meck:expect(
    edts_dist,
    start_service,
    fun (foo, _Srv) -> ok; (bar, _Srv) -> {error, already_started} end
  ),
  ...
  meck:expect(edts_dist, init_node, fun (foo, _) -> error(some_error) end),
  Call2 = {init_node, project_name, N1#node.name, "", [], [], [], undefined},
  ?assertEqual(
    {reply, {error, some_error}, S1},
...
nodes_test() ->
  ...
  S2 = #state{nodes = [N2]},
  ?assertEqual(
    {reply, {ok, [N1#node.name]}, S1},
    handle_call(nodes, {self(), foo}, S1)
  ),
  ?assertEqual(
    {reply, {ok, [N2#node.name]}, S2},
...
nodedown_test() ->
  ...
  N2 = #node{name = bar},
  S1 = #state{nodes = [N1]},
  ?assertEqual({noreply, S1}, handle_info({nodedown, N2#node.name, dummy}, S1)),
  ?assertEqual(
    {noreply, #state{}},
