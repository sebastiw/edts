...
spec(analyze, 2) -> [{out_plt, string}, {modules, [atom]}];
spec(analyze, 3) -> [{otp_plt, string}, {out_plt, string}, {modules, [atom]}].
...
-spec analyze(
    ...
    Modules :: [filename:filename()] | all
) ->
    [term()].

%%------------------------------------------------------------------------------

analyze(OutPlt, Modules) -> analyze(undefined, OutPlt, Modules).
...
-spec analyze(
    BasePlt :: filename:filename() | undefined,
    OutPlt :: filename:filename(),
    Modules :: [filename:filename()] | all
) ->
    [term()].
...
analyze(BasePlt, OutPlt, Modules) ->
    % Non-otp modules
    LoadedFiles = non_otp_beam_files(code:lib_dir(), code:all_loaded()),
    try
    ...
    case filelib:is_file(OutPlt) of
        ...
        true ->
            ...
            case ordsets:from_list(get_included_files(OutPlt)) of
                ...
                OldFileSet ->
                    Files2Add =
                        ordsets:to_list(ordsets:subtract(FileSet, OldFileSet)),
                    Files2Remove =
                        ordsets:to_list(ordsets:subtract(OldFileSet, FileSet)),
                    case Files2Add of
                        [] -> ok;
...
create_plt(BasePlt0, OutPlt, Files) ->
    BaseOpts = [{get_warnings, false}, {files, Files}, {output_plt, OutPlt}],
    BasePlt =
        case BasePlt0 of
            undefined ->
                try
                    ...
                    throw : {dialyzer_error, _} -> undefined
                end;
...
-spec non_otp_beam_files(filename:filename(), [{module(), filename:filename()}]) ->
    [filename:filename()].
...
non_otp_beam_file_p(OtpLibDir, M, Loc) ->
    ...
    lists:member(debug_info, Opts)
    andalso filename:extension(Loc) =:= ".beam"
    andalso not lists:prefix(OtpLibDir, Loc).
                    ...
                    [
                        ...
                        },
                        {
                            ...
                            ?_assertEqual(
                                ...
                                begin
                                    ok =
                                        update_plt("", File, ["file1", "file3"]),
                                    Hist = meck_history(dialyzer, run, 1),
                                    [{[Arg4], ok}, {[Arg5], ok}] = Hist,
                                    Type1 =
                                        lists:keyfind(analysis_type, 1, Arg4),
                                    Type2 =
                                        lists:keyfind(analysis_type, 1, Arg5),
                                    {Type1, Type2}
                                end
                            )
                        }
