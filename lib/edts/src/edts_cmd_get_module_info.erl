...
spec() -> [nodename, module, info_level].
...
format(Info) -> lists:foldl(fun format/2, [], Info).

format({exports, Exports}, Acc) -> [{exports, Exports} | Acc];
format({source, Source}, Acc) -> [{source, list_to_binary(Source)} | Acc];
...
format({imports, Imports}, Acc) -> [{imports, Imports} | Acc];

format({includes, Includes}, Acc) ->
  [{includes, [list_to_binary(I) || I <- Includes]} | Acc];

format({module, _} = Module, Acc) -> [Module | Acc];
format(_, Acc) -> Acc.
